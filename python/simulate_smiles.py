...
try: 
    subprocess.check_output([f'gsutil','ls',wd+phen+f'.simulation.top{n_top_loci}loci.ldwindow{int(ld_window/1e3)}kb.mt']) != None
    sim_all_loci = hl.read_matrix_table(wd+phen+f'.simulation.top{n_top_loci}loci.ldwindow{int(ld_window/1e3)}kb.mt')
    ...
    for variant in top_variants:
        ...
        mt2 = mt2.annotate_rows(in_top_loci = hl.max(mt2.in_top_loci, (hl.int(mt2.locus.contig)==int(ch))*
    ...
    sim_all_loci = mt_all_loci.annotate_cols(y = hl.agg.sum(mt_all_loci.sim_beta * mt_all_loci.dosage)) #simulate phenotype without noise (no need to normalize genotypes)
    
    sim_all_loci = sim_all_loci.checkpoint(wd+phen+f'.simulation.top{n_top_loci}loci.ldwindow{int(ld_window/1e3)}kb.mt',overwrite=True)
