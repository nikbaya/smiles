    ...
    try:
        subprocess.check_output(
            [
                ...
                wd +
                phen +
                f'.simulation.top{n_top_loci}loci.ldwindow{int(ld_window/1e3)}kb{".block_mhc" if block_mhc else ""}.mt/_SUCCESS']) is not None
        sim_all_loci = hl.read_matrix_table(
            wd + phen + f'.simulation.top{n_top_loci}loci.ldwindow{int(ld_window/1e3)}kb{".block_mhc" if block_mhc else ""}.mt')
        ...
        try:
            subprocess.check_output(
                [
                    ...
                    wd +
                    phen +
                    f'.genotypes.top{n_top_loci}loci.ldwindow{int(ld_window/1e3)}kb{".block_mhc" if block_mhc else ""}.mt/_SUCCESS']) is not None
            mt_all_loci = hl.read_matrix_table(
                wd + phen + f'.genotypes.top{n_top_loci}loci.ldwindow{int(ld_window/1e3)}kb{".block_mhc" if block_mhc else ""}.mt')
        except BaseException:
            print('Getting mt_all_loci matrix table...')
            ...
            mt1 = mt0.filter_cols(hl.is_defined(withdrawn[mt0.s]),keep=False) #Remove withdrawn samples
            ...
            # get top variants and top loci (all SNPs within ld_window)
            top_variants = []
            ...
            mt_all_loci.write(wd+phen+f'.genotypes.top{n_top_loci}loci.ldwindow{int(ld_window/1e3)}kb{".block_mhc" if block_mhc else ""}.mt',overwrite=True)
        ...
        sim_all_loci = sim_all_loci.checkpoint(
            wd +
            phen +
            f'.simulation.top{n_top_loci}loci.ldwindow{int(ld_window/1e3)}kb{".block_mhc" if block_mhc else ""}.mt',
            overwrite=True)

    return sim_all_loci


def gwas_all_loci(sim_all_loci_mt, phen, n_top_loci, ld_window, block_mhc):
    try:
        subprocess.check_output(
            [
                f'gsutil',
                ...
                wd +
                phen +
                f'.gwas.top{n_top_loci}loci.ldwindow{int(ld_window/1e3)}kb{".block_mhc" if block_mhc else ""}.ht/_SUCCESS']) is not None
    except BaseException:
        print('\n###################')
        ...
        gwas_all_loci_ht.write(
            wd +
            phen +
            f'.gwas.top{n_top_loci}loci.ldwindow{int(ld_window/1e3)}kb{".block_mhc" if block_mhc else ""}.ht',
            overwrite=True)


def get_loci_ld(phen, n_top_loci, ld_window, block_mhc):
    ...
    ht.export(wd +
              phen +
              f'.gwas.corr.top{n_top_loci}loci.ldwindow{int(ld_window/1e3)}kb{".block_mhc" if block_mhc else ""}.tsv.gz')
