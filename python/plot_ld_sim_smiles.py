...
## Prepare data
df = pd.read_csv(smiles_wd+'data/'+phen+f'.gwas.corr.top{n_top_loci}loci.ldwindow{int(ld_window/1e3)}kb{".block_mhc" if block_mhc else ""}.tsv.gz',
                 compression='gzip',
                 sep='\t')
...
if 'locus_idx' not in df.columns.values:
    ...
    df.to_csv(smiles_wd+'data/'+phen+f'.gwas.corr.top{n_top_loci}loci.ldwindow{int(ld_window/1e3)}kb{".block_mhc" if block_mhc else ""}.tsv.gz',
                 compression='gzip',
                 sep='\t',
                 index=False)
...
#        ax.scatter(x=df_locus.sim_raf, y=df_locus.sim_rbeta,s=50,c=df_locus[f'r2_locus{locus_idx}'],
#                   marker='.',cmap='viridis',vmin=r2_threshold,vmax=1)
#        ax.plot(df.loc[df.locus_idx==locus_idx].sim_raf,df.loc[df.locus_idx==locus_idx].sim_rbeta,'rx',ms=10)
#        plt.xlim([0,1])
#        plt.title(f'Risk AF vs. Effect Size\n(locus{locus_idx}: {sentinel_chr}:{sentinel_pos} , r2>{r2_threshold})')
#        
#        fig,ax=plt.subplots(figsize=(6,4))
#        ax.scatter(x=df_locus.sim_raf, y=df_locus.sim_varexp,s=50,c=df_locus[f'r2_locus{locus_idx}'],
#                   marker='.',cmap='viridis',vmin=r2_threshold,vmax=1)
#        ax.plot(df.loc[df.locus_idx==locus_idx].sim_raf,df.loc[df.locus_idx==locus_idx].sim_rbeta,'rx',ms=10)
#        plt.xlim([0,1])
#        plt.title(f'Risk AF vs. Variance Explained\n(locus{locus_idx}: {sentinel_chr}:{sentinel_pos} , r2>{r2_threshold})')
...
#    plt.savefig(smiles_wd+'smiles/plots/'+phen+f'.prop_raf_gt_sentinel.r2_{r2_threshold}.top{n_top_loci}loci.simulated.png',dpi=300)
    ...
    plt.title(f'Proportion of variants in a locus with RAF > sentinel RAF\nvs. number of variants in locus (phen: {phen_dict[phen]}, top {int(df.locus_idx.max())+1} loci, r2>{r2_threshold})')
    plt.savefig(smiles_wd+'smiles/plots/'+phen+f'.prop_raf_gt_sentinel_vs_n_snps.r2_{r2_threshold}.top{n_top_loci}loci.simulated.png',dpi=300)
